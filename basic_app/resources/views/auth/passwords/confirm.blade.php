...
@section('content')
    ...
    <div style="background: #fff; border-radius: 24px; box-shadow: 0 8px 32px 0 rgba(31,38,135,0.12); padding: 48px 32px; max-width: 420px; width: 100%; display: flex; justify-content: center; align-items: center; min-height: 80vh;">
        <div style="text-align: center; margin-bottom: 24px;">
            ...
            <h2 style="font-size: 2rem; font-weight: 700; color: #22223B; margin-bottom: 8px;">{{ __('adminlte::adminlte.comfirm_passwors') }}  </h2>
            <p style="color: #888; font-size: 1rem;">{{ __('adminlte::adminlte.please_confirm_continuing_before') }}</p>
        </div>
        @if (session('error'))
        ...
        <form method="POST" action="{{ route('password.confirm') }}">
            ...
            <div style="margin-bottom: 18px;">
                <label for="password" style="display:block; color:#22223B; font-weight:500; margin-bottom:6px;">{{ __('adminlte::adminlte.confirm_password') }}</label>
...
{{ __('adminlte::adminlte.confirm_password') }}            </button>
        </form>
        <div style="text-align:center; color:#888; font-size:15px; margin-top:18px;">
            <a href="{{ route('password.request') }}" style="color:#6C63FF; font-weight:600;">{{ __('adminlte::adminlte.forgot_password') }}</a>
        </div>
